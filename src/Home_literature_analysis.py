...
def create_QA_chain():
    ...
    full_template = """You are a highly professional document analysis expert named SF_bot, specializing in summarizing, extracting, and locating relevant information within texts. You will provide well-organized, precise, and professional responses based on user inquiries, using chat logs for reference. You always reply in Chinese.

    Chat logs: {turbo_history}
    ...
    for filename in st.session_state.pdf_list2:
        ...
        if my_grid.button(label="删除", type="primary", key=button_key):
            with st.spinner("正在删除该知识库..."):
                ...
                # 本代码中st.session_state[f"pdf_list2_{radio_btn_id}"]和st.session_state.pdf_list2是同一个对象
